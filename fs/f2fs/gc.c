				...
				int gc_type, unsigned int segno, int off)
{
	...
	if (!down_write_trylock(&inode->i_mapping->invalidate_lock))
		return -EAGAIN;
	...
	if (err)
		goto out_recover;
	map_len = dax_direct_access(dax_dev, pgoff, 1,
				DAX_ACCESS, &old_addr, &pfn);
	if (map_len < 0) {
		err = map_len;
		goto out_recover;
	}
...
move_data:
			...
			if (!err && (gc_type == FG_GC || IS_DAX(inode) ||
